...
from .elementosNivel import NieblaNivel, RocaImpulso
from .escenarioNivel import EscenarioNivel
from .recursosNivel import COLORES_TERRENO, TINTES_TERRENO
from .plataformaNivel import TipoDecoracion
...
class DibujadorEscenario:
    ...
    def __init__(self, pantalla: pygame.Surface, recursos) -> None:
        ...
        imagenes = CargadorRecursosNivel(Path(__file__).resolve().parents[1]).cargar(
            recursos
        )
    ...
    def dibujarEnemigos(self, enemigos, camara):
        for enemigo in enemigos:
            ...
            if enemigo.liberado:
                ...
                self.pantalla.blit(self.spritesEnemigos[enemigo.tipo], (x - 20, y - 12))
            else:
                pygame.draw.circle(self.pantalla, (61, 44, 79), (x + 16, y + 20), 20)
            if enemigo.tipo == TipoEnemigo.ESPEJILLA:
