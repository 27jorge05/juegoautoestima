...
from src.dominio.dominio import EstadoNivel
...
class DibujadorNivelUno:
    ...
    def __init__(self, pantalla: pygame.Surface, recursosNivel) -> None:
        ...
        carpetaPersonajes = Path(__file__).resolve().parents[2] / "assets" / "characters" / "rumi"
    ...
    def actualizar(self, nivel, deltaTiempo):
        ...
        rumi = nivel.rumi
        tiempoAnimacion = 0.0 if nivel.terremotoIniciado else deltaTiempo
        self.animacionRumi.actualizar(obtenerAnimacionRumi(rumi.velocidad.x, rumi.velocidad.y, rumi.estaEnSuelo, rumi.garrasActivas), tiempoAnimacion)
        self.animacionPadre.actualizar("reposo" if nivel.secuencia.estado == EstadoNivel.SEGUIR_PADRE else "guiar", tiempoAnimacion)
    ...
    def dibujar(self, nivel: NivelUno, desplazamientoCamara: float) -> None:
        ...
        self.dibujarDialogosCriaturas(nivel, desplazamientoCamara)
    ...
    def dibujarDialogosCriaturas(self, nivel: NivelUno, desplazamientoCamara: float) -> None:
        """El diálogo procede de cada criatura, nunca del nivel ni del dibujador."""
        for enemigo in nivel.escenario.enemigos:
            if not enemigo.emiteFrase or enemigo.estado != EstadoEnemigo.AVISO:
                continue
            x, y = round(enemigo.posicion.x - desplazamientoCamara), round(enemigo.posicion.y)
            if not -80 < x < ANCHO_VENTANA + 80:
                continue
            dibujarBurbujaDialogoCriatura(
                self.pantalla, self.fuente, enemigo.dialogoAviso.textoMostrado, x + 21, y
            )
    ...
    def dibujarFinal(self, nivel: NivelUno) -> None:
        ...
        textos = (
            "¡Lo encontraste!",
            "Mr. Fox y Rumi seguirán hacia el farol que quedó al otro lado.",
    ...
    def dibujarInterfazFija(self, nivel: NivelUno) -> None:
        """Restaura el panel narrativo después de sacudir solamente el mundo."""
        ancho, alto = self.pantalla.get_size()
        pygame.draw.rect(self.pantalla, (9, 10, 20), (0, alto - 154, ancho, 154))
        self.dibujarInterfaz(nivel)
